...
(function () {
  ...
  if (form) {
    ...
    function showThanks(queued) {
      ...
      if (note) {
        note.innerHTML = queued
          ? "✅ We saved your details. Our team will call you soon. If not, please call or WhatsApp <b>86229 09192</b>."
          : noteOkHtml;
        note.hidden = false;
    ...
    form.addEventListener("submit", function (e) {
      ...
      // tab; the server dedupes by mobile). If the save truly fails, queueLead keeps
      // it for automatic retry and showThanks(true) gives the customer our number —
      // the lead is preserved either way, never lost behind a false "thank you".
      var btn = form.querySelector('[type="submit"]');
      if (btn) { btn.disabled = true; }
      var endpoint = API_BASE + "/api/dealership";
      postLead(endpoint, data)
        .then(function () { showThanks(false); })                            // server confirmed
        .catch(function () { queueLead(endpoint, data); showThanks(true); }) // preserve + retry later
